$(document).ready(function(){
    ...
    $('#signupForm').submit(function(event){
        ...
        // alert(firstname + "<br />" + lastname + "<br />" + email + "<br />" + password + "<br />" + confirmpassword);
	      ...
	      success: function(msg) {
	        if (msg == 'OK') {
				...
				for(val in msg){
					...
					if(msg[val].param == 'lastname'){
						$('#lastname_text').slideDown('slow');
						$('#lastname_text').append('&times; ' + msg[val].msg + '<br />');
					}
	...
	$('#updateProfiles').click(function(event){
        event.preventDefault();
        ...
        $.ajax({
            ...
            console.log(result);
            if(result){
                $('#updateprofile').attr('action','/update/' + result._id + '?_method=PUT');
				$('#updateprofile input:eq(1)').val(result.firstname);
				$('#updateprofile input:eq(2)').val(result.lastname);
				$('#updateprofile input:eq(3)').val(result.email);
                $('#updateModal').modal('show');
            }
        });
    });
